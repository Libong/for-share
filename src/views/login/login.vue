<template>
  <div class="login-container">
    <div class="form-container">
      <div v-if="loginViewType == LoginViewType.LOGIN" class="login-view">
        <Bell
        ...
        <div class="to-forget">
          ...
          <span :style="jumpSpanStyle" @click="toForget">忘记密码?</span>
        </div>
        <LoginButton :text="loginChangeItem.loginType === FormTypeEnum.Phone?'注 册 / 登 录':'登 录'" @click="login"/>
      </div>
      <div v-else-if="loginViewType == LoginViewType.FORGET" class="forget-view">
        <Forget
            @backToLogin="toLogin">
        </Forget>
      </div>
      <div v-else-if="loginViewType == LoginViewType.REGISTER" class="register-container">
        <img :src="BackArrow" alt="" class="back-arrow" @click="toLogin"/>
        ...
        <CustomInput
            ...
            :type="'password'"
        />
        <LoginButton :text="'注  册'" style="" @click=""/>
      </div>
    </div>
  </div>
</template>
...
let emailEl = ref();
/*------登录功能切换------*/
let loginViewType = ref(LoginViewType.LOGIN);
...
//去注册页面
function toRegister() {
  loginViewType.value = LoginViewType.REGISTER;
  ObjClear(loginReq);
}

//去登录页面
function toLogin() {
  if (loginViewType.value == LoginViewType.REGISTER) {
    //关闭提示红字
    confirmPasswordEl.value.showNotice(false);
  }
  loginViewType.value = LoginViewType.LOGIN;
  ObjClear(registerReq);
}

//去忘记密码页面
async function toForget() {
  loginViewType.value = LoginViewType.FORGET;
  ObjClear(loginReq);
}
